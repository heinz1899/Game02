...
def würfeln():
    for e in group_elemente:
        if e.typ != 'Würfel' or e.selektiert:
            continue
...
bild_würfel_e = [pg.image.load(f'{pfad}dices/{n+1}_e.png') for n in range(6)]
bild_würfel_d = [pg.image.load(f'{pfad}dices/{n+1}_d.png') for n in range(6)]
group_elemente = pg.sprite.Group()

for n in range(1, 6):
    pos = (606, 130 + 90 * (n - 1))
    group_elemente.add(Element(n, 'Würfel', f'{pfad}dices/{n}', pos, True))

for n in range(3):
    pos = [(400, 615), (815, 615), (506, 450)]
    enabled = n == 0
    group_elemente.add(Element(n, 'Button', f'{pfad}buttons/Button{n}', pos[n], enabled))
...
while weitermachen:
    ...
    for ereignis in pg.event.get():
        ...
        if ereignis.type == pg.MOUSEBUTTONDOWN:
            ...
            for e in group_elemente:
                if not e.enabled or not e.rect.collidepoint(point):
                    continue
                if e.typ == "Button":
                    clicked_button(e)
                if e.typ == "Würfel":
